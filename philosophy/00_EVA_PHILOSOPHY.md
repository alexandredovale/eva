...
Similaridade é um mecanismo de ordenação, não um juízo de verdade, importância ou força cognitiva. Nas rotas vetoriais, toda a população primária elegível manifesta sua geometria antes que κq estabeleça uma fronteira query-local. O primeiro Context Intelligence Engine observa, por média, desvio padrão e coeficiente de variação, essa população legitimada e mantém o núcleo `s ≥ μ + σ` como corte, usando convergência somente como fallback. κe e CIE primário preservam os cálculos posteriores sobre as fontes sobreviventes; por fim, o CIE global observa a união dos núcleos locais. Seu núcleo continua sendo a base principal e sua convergência `μ ≤ s < μ + σ` é anexada como contexto auxiliar, sem alterar κq, κe ou o corte. Na base final permanecem somente as fontes que o modelo incorporou à resposta com citação visível. Valores e análises estatísticas são descartados após a consulta e nunca se tornam memória, peso ou relação permanente.
...
Recuperar um candidato não equivale a mantê-lo como fundamento final. O Retriever ordena a população primária completa; κq legitima sua fronteira; o primeiro CIE encaminha o núcleo superior ou sua convergência de fallback; κe e CIE primário elegem núcleos locais; e o CIE global consolida sua união sem quantidade configurada. A IA recebe o núcleo global como base principal e sua convergência como contexto auxiliar, não pode introduzir fontes externas e pode omitir candidatas que não contribuam para a prosa. O papel global `core` ou `convergence` acompanha cada fonte, âncoras literais exatas permanecem protegidas e somente as evidências efetivamente citadas permanecem na base final.

...
> Estado atual na versão 7.0.0: `QUERY_CANDIDATE_LIMIT` permanece removido. A fronteira κq emerge da população primária completa de cada consulta, o corte intermediário continua no núcleo e a convergência global é acrescentada somente como contexto auxiliar. A captura, a fila e a consulta não geram nem consomem resumos derivados. Os valores acima e a validação de 8 de agosto permanecem apenas como registros históricos do fluxo anterior.

> Validação dirigida em 9 de setembro de 2026: a pergunta “Quais são os princípios fundamentais do Espiritismo?” foi executada sobre as cinco obras do projeto local *Pentateuco Espírita* com uma chamada real paga. κq e os cortes intermediários permaneceram inalterados. O CIE global recebeu 40 núcleos locais, classificou 7 como `core`, 15 como `convergence` e descartou 18. O contexto autorizado reuniu o núcleo e a convergência globais; a resposta validada citou seis fontes, cinco `core` e uma `convergence`, em 16,83 segundos, sem limitação de cobertura documental. Isso valida funcionalmente a convergência como subsídio auxiliar, sem promovê-la a núcleo ou alterar as fronteiras κ.

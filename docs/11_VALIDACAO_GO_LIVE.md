...
> Estado atual da versão 4.0.2: `QUERY_CANDIDATE_LIMIT` permanece removido. A fronteira κq agora emerge da população primária completa de cada consulta, e o primeiro CIE encaminha somente o núcleo superior ou seu fallback de convergência. Os valores acima e a validação de 8 de agosto permanecem apenas como registros históricos do fluxo anterior orientado por resumos.

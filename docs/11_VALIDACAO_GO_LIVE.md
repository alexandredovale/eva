...
> Estado atual da versão 6.0.0: `QUERY_CANDIDATE_LIMIT` permanece removido. A fronteira κq emerge da população primária completa de cada consulta, e o primeiro CIE encaminha somente o núcleo superior ou seu fallback de convergência. A captura, a fila e a consulta não geram nem consomem resumos derivados. Os valores acima e a validação de 8 de agosto permanecem apenas como registros históricos do fluxo anterior.

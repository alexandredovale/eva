...
> Current state in version 7.0.0: `QUERY_CANDIDATE_LIMIT` remains removed. The κq boundary emerges from each query's complete primary population, intermediate selection remains core-based, and global convergence is appended only as auxiliary context. Capture, queueing, and querying neither generate nor consume derived summaries. The values above and the August 8 validation remain historical records of the former flow.

> Directed validation on September 9, 2026: the question “Quais são os princípios fundamentais do Espiritismo?” was run over the five works in the local *Pentateuco Espírita* project with one real paid call. κq and all intermediate cutoffs remained unchanged. Global CIE received 40 local nuclei, classified 7 as `core`, 15 as `convergence`, and discarded 18. Authorized context combined global core and convergence; the validated answer cited six sources—five `core` and one `convergence`—in 16.83 seconds, without a documentary-coverage limitation. This functionally validates convergence as auxiliary support without promoting it to core or changing κ boundaries.

...
> Current version 6.0.0 state: `QUERY_CANDIDATE_LIMIT` remains removed. The κq boundary emerges from each query's complete primary population, and the first CIE forwards only the upper core or its convergence fallback. Capture, queueing, and querying neither generate nor consume derived summaries. The values above and the August 8 validation remain historical records of the former flow.

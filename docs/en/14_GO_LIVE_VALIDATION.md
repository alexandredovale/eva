...
> Current version 4.0.2 state: `QUERY_CANDIDATE_LIMIT` remains removed. The κq boundary now emerges from each query's complete primary population, and the first CIE forwards only the upper core or its convergence fallback. The values above and the August 8 validation remain historical records of the former summary-first flow.
